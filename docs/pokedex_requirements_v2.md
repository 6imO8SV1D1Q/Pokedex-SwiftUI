...
| バージョン | 日付 | 変更内容 |
|----------|------|---------|
| 2.0.0 | 2025-10-05 | 初版作成。世代別表示、全ポケモン対応、フィルター・ソート機能拡張 |
...
- 特性名が長い場合は省略表示を検討
- 注意事項:
  - 第1〜2世代: 特性システムが存在しないため「-」表示
  - 第3〜4世代: 隠れ特性が存在しないため通常特性のみ
...
- **全国図鑑モードでの動作:** 
  ...
  - ポケモンの特性は世代によって変わるが、全国図鑑では最新仕様を採用
...
- **世代選択済みの場合:**
  - 特性フィルターは有効
  - 選択した世代における特性のみ選択肢に表示
  - 第1〜2世代選択時は特性フィルターを無効化(特性システムが存在しないため)
...
- [ ] 第1〜2世代選択時は特性フィルターが無効化される(特性システム未実装のため)
- [ ] 無効化時に適切なメッセージが表示される
...
#### 4.1 概要
ポケモンの世代を選択し、その世代における仕様(タイプ、特性、覚える技など)でポケモン情報を表示する機能。

#### 4.2 背景・目的
- **現状の課題:** 
  - タイプや特性が世代によって異なる場合があるが、最新仕様のみ表示
  - 歴代のゲーム体験と情報が一致しない
- **改善の価値:** 
  - 各世代のゲーム仕様に即した正確な情報提供
  - ユーザーが遊んだ世代の情報を確認できる
...
| 世代 | 対象ゲーム | ポケモン数(累計) | 主な変更点例 |
|-----|----------|----------------|------------|
| 第1世代 | 赤・緑・青・ピカチュウ | 151 | - |
| 第2世代 | 金・銀・クリスタル | 251 | あく・はがねタイプ追加、特性システム未実装 |
| 第3世代 | ルビー・サファイア・エメラルド | 386 | 特性システム導入 |
| 第4世代 | ダイヤモンド・パール・プラチナ | 493 | 物理・特殊の仕様変更 |
| 第5世代 | ブラック・ホワイト | 649 | 隠れ特性導入 |
| 第6世代 | X・Y | 721 | フェアリータイプ追加、メガシンカ |
| 第7世代 | サン・ムーン | 809 | Zワザ |
| 第8世代 | ソード・シールド | 905 | ダイマックス |
| 第9世代 | スカーレット・バイオレット | 1025 | テラスタル |

**4.3.2 世代選択UI**
- 一覧画面上部に世代セレクター追加
- セグメントコントロールまたはドロップダウン形式
- **選択肢:**
  - 「全国図鑑」(全世代のポケモンを表示、デフォルト)
  - 「第1世代」〜「第9世代」

**デフォルト動作:**
- 初回起動時: 「全国図鑑」モードで起動
- **全国図鑑モードは最新世代(第9世代)と同等**
  - 全ポケモン(1〜1025)を表示
  - タイプ、特性、種族値はすべて最新仕様(第9世代時点)
  - **タイプフィルター・特性フィルターは有効**: 第9世代仕様として扱う
  - **技フィルターのみ無効**
    - 理由: 技の習得方法は世代ごとに大きく異なり、「第9世代で習得可能」と明示する必要があるため
    - ユーザーに世代選択を促すメッセージを表示: 「技で絞り込むには世代を選択してください」

**世代選択時の表示:**
- その世代までに登場したポケモンのみ表示
- タイプ、特性、種族値はすべて選択世代での仕様を表示
...
- 特性(例: ゲンガーは第3〜6世代は「ふゆう」、第7世代以降は「のろわれボディ」)
- 種族値(まれに調整が入る)
- 覚える技

**4.3.4 表示範囲**
- **全国図鑑モード:** 全ポケモン(1〜1025)を表示
- **世代選択時:** 選択した世代までに登場したポケモンのみ表示
  - 例: 第1世代選択時は001〜151のみ、第2世代選択時は001〜251
...
#### 4.5 受け入れ基準
- [ ] 世代セレクターが表示される
- [ ] 世代を切り替えると、その世代のポケモンのみ表示される
- [ ] 世代別のタイプ・特性が正しく表示される
- [ ] 世代切り替え時にローディングが表示される
- [ ] デフォルトは第9世代(最新)
...
#### 5.1 概要
現在の第1世代151匹から、全世代のポケモン約1025匹に対応を拡大する。

#### 5.2 背景・目的
- **現状の課題:** 第1世代のみ対応、ユーザーの選択肢が限定的
- **改善の価値:** 
  - 全ポケモンを網羅し、あらゆる世代のファンに対応
  - 世代別表示機能の前提条件

#### 5.3 機能詳細

**5.3.1 対応範囲**
- 第1世代〜第9世代のポケモン(図鑑番号1〜1025)
...
**段階的ロード戦略:**
1. **初回表示:** 選択世代のポケモンのみ取得(例: 第1世代なら151匹)
2. **バックグラウンドロード:** 残りの世代を優先度順に取得
...
- **世代未選択(全国図鑑モード)の場合:**
  ...
  - 「世代を選択してください」とメッセージ表示
  
- **世代選択済みの場合:**
  - 技フィルターが有効化
  - 選択した世代で存在する技のみ選択肢に表示
  - 選択した世代での習得可否を判定
...
**7.3.6 世代切り替え時の動作**
- 世代を変更すると、選択済み技フィルターは自動クリア
- 理由: 前の世代で選択した技が新しい世代に存在しない可能性があるため
...
- [ ] 各技の習得方法が表示される
- [ ] 選択中の世代で存在する技のみ選択肢に表示される
- [ ] 選択中の世代での習得可否が正しく判定される
- [ ] フィルタークリアで技フィルターもクリアされる
- [ ] 世代切り替え時、選択済み技フィルターがクリアされる
...
| ポケモン一覧データ | メモリキャッシュ(Dictionary) | セッション中 | 世代ごとに分離 |
